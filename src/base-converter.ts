...
export class BaseConverter {
  ...
  private validateNumberInBaseN(
    ...
    if (
      splitterdNumberByDelimiter.length < 1 ||
      splitterdNumberByDelimiter.length > 2
    ) {
  ...
  private convertIntegerPart(integerPart: number, baseTo: number) {
    ...
    while (true) {
      ...
      remainders.push(
        remainder >= 10
          ? this.numberToLettersMap[remainder]
          : remainder.toString(),
      );
    ...
    let result =
      integerPart >= 10
        ? (this.numberToLettersMap[integerPart] as string)
  ...
  private convertDecimalPart(
    ...
    for (let index = 0; index < precision; index++) {
      ...
      result +=
        decimalPartCopy >= 10
          ? this.numberToLettersMap[integerPart]
          : integerPart.toString();
      decimalPartCopy %= 1;
    }
